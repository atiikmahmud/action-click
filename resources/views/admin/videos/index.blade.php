        ...
        <div class="card">
            <div class="d-flex justify-content-between">
                ...
                <div class="nav-item d-flex align-items-center desktop-search-view">
                    ...
                    @if ($search)
                        <a href="{{ route('admin.videos.index') }}"><i class="ri-delete-back-2-line"></i></a>
                    @endif
                </div>
                <!-- /Search -->
                <h5 class="card-header">
                    <a href="{{ route('admin.video.add') }}" class="btn btn-primary">
                        <span class="tf-icons ri-add-circle-line ri-16px me-1_5"></span>Upload Video
                    </a>
                </h5>
            </div>
            <div class="table-responsive text-nowrap">
                <table id="image-table" class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                            ...
                            @foreach ($videos as $index => $video)
                                <tr>
                                    ...
                                    <td><img src="{{ asset('admin-assets/img/avatars/1.png') }}"
                                            ...
                                            data-uploadon="{{ date('d M Y', strtotime($video->created_at)) }}"
                                            data-uploadby="{{ $video->user->name }}"
                                            data-status="{{ ucfirst($video->status) }}
                                                @if ($video->status == 'new') (Need approval) @endif" />
                                    ...
                                    <td>
                                        @if ($video->featured == 1)
                                            <a href="{{ route('admin.video.featured', ['id' => $video->id]) }}">
                                            ...
                                            <div class="dropdown-menu">
                                                ...
                                                <form action="#" method="POST">
                                                    ...
                                                    <button type="submit" class="dropdown-item"
                                                        onclick="return confirm('Are you sure delete this video ?')"><i
                        ...
                        @else
                            <tr>
                                <td class="text-center" colspan="6">Video List Empty...</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
                ...
                <div class="mt-5">
                    {{ $videos->withQueryString()->links('vendor.pagination.default') }}
                </div>
            </div>
        </div>
    ...
    <script>
        $(function() {
            ...
            $('#approve-loader').click(function() {
                const id = $(this).data('id');
                var url = "{{ route('admin.video.approved', ['id' => ':id']) }}";
            ...
            $('#reject-loader').click(function() {
                const id = $(this).data('id');
                var url = "{{ route('admin.video.rejected', ['id' => ':id']) }}";
                url = url.replace(':id', id);
